...
import EditEventModal from './EditEventModal'; // Ajustez le chemin en fonction de votre structure de fichiers
...
const Evenement: React.FC = () => {
  ...
  const fetchEvent = async (page: number, size: number, search: string) => {
    setLoading(true);
    try {
      const response = await axiosInstance.get(`/api/evenements/president/${user?.userId}`, {
        params: {
          page: page,
          size: size,
...
// Utiliser useEffect pour récupérer les utilisateurs à chaque changement de page, taille ou terme de recherche
  ...
  if (file && file.size > MAX_FILE_SIZE) {
    Swal.fire({
      icon: 'error',
      title: 'Erreur',
      text: `Le fichier  ${documentType} dépasse la taille maximale autorisée de 30 Mo.`,
    });
    return; // Sortir de la fonction si le fichier est trop volumineux
  }
    ...
    switch (documentType) {
      case 'Affiche':
        apiUrl = `/api/evenements/affiche/${evenementId}`;
        break;
      case 'Programme Prévisionnel':
        apiUrl = `/api/evenements/progPrev/${evenementId}`;
        break;
      case 'Liste Intervenants':
        apiUrl = `/api/evenements/listInter/${evenementId}`;
        break;
      case 'Compte Rendu':
        apiUrl = `/api/evenements/compteRendu/${evenementId}`;
        break;
      case 'Document Autorité':
        apiUrl = `/api/evenements/documentAutorite/${evenementId}`;
        break;
      default:
        return;
    }
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire(
          'Succès!',
          `${documentType} modifié avec succès !`,
          'success'
        );
...
const refreshDocuments = async (evenementId: number) => {
  try {
    const response = await axiosInstance.get(`/api/evenements/${evenementId}`);
