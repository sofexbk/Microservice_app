...
const EvenementAtt: React.FC = () => {
  ...
  const formatDate = (dateString: string) => {
    ...
    const handleRefuseConfirm = async () => {
      if (selectedEventId && refusalReason.trim()) {
        ...
        try {
          const response = await axiosInstance.post(`/api/evenements/refuser/${selectedEventId}/${user?.userId}`,null, 
           { params:{
    ...
    const handleAccept = async (eventId: number) => {
      ...
      if (eventId && user?.userId) {
        try {
          const response = await axiosInstance.post(`/api/evenements/accepter/${eventId}/${user?.userId}`);
          fetchEvent(pageNumber, itemsPerPage, searchTerm);
          setSnackbarMessage(response.data);
          setSnackbarSeverity('success');
          setSnackbarOpen(true); // Ouvrir le Snackbar
  ...
  const fetchEvent = async (page: number, size: number, search: string) => {
    setLoading(true);
    try {
      const response = await axiosInstance.get('/api/evenements/encours/parrainId', {
        params: {
          page: page,
          size: size,
  ...
  return (
    <>
      ...
      <div className="h-16 w-16 animate-spin rounded-full border-4 border-solid border-primary border-t-transparent">
          ...
          <p className="text-center">Aucun événement en attente de validation.</p>
        ) : (
            <>
