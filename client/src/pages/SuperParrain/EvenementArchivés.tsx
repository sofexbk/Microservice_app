...
const EvenementArchivés: React.FC = () => {
  ...
  const fetchEvent = async (page: number, size: number) => {
    setLoading(true);
    try {
      const response = await axiosInstance.get('/api/evenements/archived-events', {
        params: {
          page: page,
          size: size,
        }
...
useEffect(() => {
  fetchEvent(pageNumber, itemsPerPage);
  ...
  return (
    <>
      ...
      <div className="h-16 w-16 animate-spin rounded-full border-4 border-solid border-primary border-t-transparent">
          ...
          <p className="text-center">Aucun événement archivé</p>
        ) : (
            <>
                <table className="w-full table-auto">
                  <thead>
                    ...
                    <th 
                        ...
                        className="min-w-[80px] py-4 px-2 font-medium text-black dark:text-white cursor-pointer text-xs sm:text-sm"
                      >
                       Date d'archivage {sortField === 'createdAt' ? (sortOrder === 'asc' ? '↑' : '↓') : ''}
                    </th>
                    <th                        
                        className="min-w-[80px] py-4 px-2 font-medium text-black dark:text-white cursor-pointer text-xs sm:text-sm"
                      >
...
export default EvenementArchivés;
