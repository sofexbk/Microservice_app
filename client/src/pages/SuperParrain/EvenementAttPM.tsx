...
const EvenementAttPM: React.FC = () => {
  ...
  const formatDate = (dateString: string) => {
    ...
    const handleRefuseConfirm = async () => {
      if (selectedEventId && refusalReason.trim()) {
        try {
          const response = await axiosInstance.post(`/api/evenements/${selectedEventId}/enplacerefuser`,null, 
           { params:{
            motifRefus: refusalReason,
            superParrainId:user?.userId,
    ...
    const handleAccept = async (eventId: number) => {
      ...
      if (eventId && user?.userId) {
        try {
          const response = await axiosInstance.post(`/api/evenements/${eventId}/enplaceaccepter`,null,
            { params:{
                superParrainId:user.userId,
                evenementId:selectedEventId
            }
  ...
  const fetchEvent = async (page: number, size: number, search: string) => {
    setLoading(true);
    try {
      const response = await axiosInstance.get('/api/evenements/encours/parrain', {
        params: {
          page: page,
          size: size,
