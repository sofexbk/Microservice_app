...
const EditEvent: React.FC<EditEventModalProps> = ({ open, onClose, evenement, fetchEvent, pageNumber, itemsPerPage, searchTerm }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        await axiosInstance.put(`/api/evenements/update/externe/${values.id}`, formData);
        Swal.fire({
          title: 'Succès',
  ...
  useEffect(() => {
    ...
    if (evenement) {
      formik.setValues({
        ...
        soutienLogistique: evenement.soutienLogistique,
        publicCible: evenement.publicCible,
        structure:evenement.structureName
      });
    }
  }, [evenement]);
  const addDate = () => {
