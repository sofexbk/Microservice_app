...
const DeposerSuper: React.FC = () => {
  ...
  const fetchEvent = async (page: number, size: number, search: string) => {
    ...
    try {
      ...
      const response = await axiosInstance.get('/api/evenements/externecp', {
        params: {
          page: page,
          size: size,
    ...
    if (file && selectedEventId) {
      ...
      try {
        const response = await axiosInstance.put(
          `/api/evenements/${selectedEventId}/deposercrex`,
          formData,
          {
            headers: {
