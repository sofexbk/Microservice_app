...
const EvenementAttSup: React.FC = () => {
  ...
  const formatDate = (dateString: string) => {
    ...
    const handleArchive = async (eventId: number) => {
      const result = await Swal.fire({
        title: 'Êtes-vous sûr ?',
        text: "Vous allez archiver cet événement !",
        icon: 'warning',
      ...
      if (eventId && user?.userId) {
        try {
          const response = await axiosInstance.post(`/api/evenements/archive/${eventId}`);
  ...
  const fetchEvent = async (page: number, size: number, search: string) => {
    setLoading(true);
    try {
      const response = await axiosInstance.get('/api/evenements/valides', {
        params: {
          page: page,
          size: size,
...
// Utiliser useEffect pour récupérer les utilisateurs à chaque changement de page, taille ou terme de recherche
  ...
  const handleDelete = (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axiosInstance.delete(`/api/evenements/${id}`);
          Swal.fire(
            'Supprimé !',
